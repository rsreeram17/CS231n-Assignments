...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  next_h = np.tanh(a)
  cache = [a, next_h, x, prev_h, Wx, Wh, b]
...
def rnn_step_backward(dnext_h, cache):
  ...
  a, next_h, x, prev_h, Wx, Wh, b = cache
  ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  N, T, D = x.shape
  _, H = h0.shape
  h, cache_step = np.zeros((T, N, H)), None
  cache = []

  # Swap axis to make processing easier
  x = np.swapaxes(x, 0, 1)

  # for each time step calculate next h
  for t in range(T):
    if t == 0:
      prev_h = h0
